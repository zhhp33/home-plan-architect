...
const ProductLibrary: React.FC<ProductLibraryProps> = ({
  ...
  return <div className="flex flex-col h-full bg-app-panel text-white">
      <div className="p-3 border-b border-gray-700/30">
        
        <div className="relative mb-3">
          <Search className="absolute left-2.5 top-2.5 h-4 w-4 text-gray-400" />
          <Input placeholder="搜索产品..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9 bg-gray-800/40 border-gray-700/30 text-white placeholder:text-gray-400" />
          {searchTerm && <button className="absolute right-2.5 top-2.5" onClick={() => setSearchTerm('')}>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        <div className="grid grid-cols-1 gap-2">
          {filteredProducts.map(product => <div key={product.id} className="product-item" draggable onDragStart={e => handleDragStart(e, product)}>
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-gray-700/50 rounded flex items-center justify-center">
                  <img src={product.thumbnail} alt={product.name} className="w-8 h-8 opacity-70" />
                </div>
                <div>
